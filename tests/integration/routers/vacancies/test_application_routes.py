...
async def test_list_and_get_application(client, session, make_user):
    ...
    session.add(vacancy)
    await session.flush()
    application = ApplicationModel(applicant_id=user.id, vacancy_id=vacancy.id, status=ApplicationStatus.pending)
    ...
    detail = await client.get(f"/api/v1/vacancies/applications/{application.id}")
...
async def test_get_application_not_found(client):
    res = await client.get("/api/v1/vacancies/applications/00000000-0000-0000-0000-000000000000")
