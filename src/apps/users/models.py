...
class ResumeModel(BaseModel, WithLocation):
    ...
    salary_expectation_min: Mapped[Optional[int]] = mapped_column(Integer, index=True)
    salary_expectation_max: Mapped[Optional[int]] = mapped_column(Integer, index=True)
    salary_currency: Mapped[Optional[SalaryCurrency]] = mapped_column(Enum(SalaryCurrency, name="salary_currency"))
    work_format: Mapped[Optional[WorkFormat]] = mapped_column(Enum(WorkFormat, name="work_format"))
    employment_type: Mapped[Optional[EmploymentType]] = mapped_column(Enum(EmploymentType, name="employment_type"), index=True)
...
class UserModel(BaseModel, WithLocation):
    ...
    password_hash: Mapped[Optional[str]] = mapped_column(String(length=255))
    ...
    headline: Mapped[Optional[str]] = mapped_column(String(120))
    ...
    # Specialization
    specialization: Mapped[Optional[Specialization]] = mapped_column(Enum(Specialization, index=True, name="specialization"))
