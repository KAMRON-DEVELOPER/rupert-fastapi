...
class FeedTagLink(BaseModel):
    ...
    feed: Mapped[FeedModel] = relationship(
        back_populates="tag_links", overlaps="feeds, tags"
    )
    tag: Mapped[TagModel] = relationship(
        back_populates="feed_links", overlaps="feeds"
    )
...
class FeedEngagementModel(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_id", "feed_id", "type", name="uq_user_feed_engagement"
        ),
    )
    ...
    type: Mapped[FeedEngagementType] = mapped_column(
        Enum(FeedEngagementType, name="feed_engagement_type")
    )

    # Relationships
    user: Mapped[UserModel] = relationship(
        back_populates="feed_engagements", passive_deletes=True
    )
    feed: Mapped[FeedModel] = relationship(
...
class FeedModel(BaseModel):
    ...
    comment_policy: Mapped[CommentPolicy] = mapped_column(
        Enum(CommentPolicy, name="comment_policy"),
        default=CommentPolicy.everyone,
    )
